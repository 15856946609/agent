...
export default function Home() {
  ...
  const generateMockQuote = useCallback((targetSymbol: string) => {
    ...
    return { c: newPrice, d: diff, dp: diffPercent, h: Math.max(newPrice, prevClose) * 1.01, l: Math.min(newPrice, prevClose) * 0.99, o: prevClose * (1 + (Math.random() - 0.5) * 0.01), pc: prevClose };
  }, []);
    ...
    try {
      ...
      if (!isChinaStock) {
        ...
        if (qData.c && qData.c !== 0) {
          const currentPrice = qData.c;
          const pc = qData.pc || currentPrice;
          const diff = currentPrice - pc;
          const dp = (diff / pc) * 100;
          
          if (prevPrice.current !== 0 && targetSymbol === symbol) {
            if (currentPrice > prevPrice.current) setPriceFlash('up');
            else if (currentPrice < prevPrice.current) setPriceFlash('down');
            setTimeout(() => setPriceFlash(null), 500);
          }
          setQuote({ ...qData, d: diff, dp: dp });
          prevPrice.current = currentPrice;
        } else {
          const mq = generateMockQuote(targetSymbol);
          setQuote(mq);
          prevPrice.current = mq.c;
        }
        if (forceCandle || loading) {
          ...
          } else setCandles(generateMockCandles(res, targetSymbol));
        }
      } else {
        const mq = generateMockQuote(targetSymbol);
        setQuote(mq);
  ...
  useEffect(() => {
    if (quote) {
      setMarketQuotes(prev => ({ ...prev, [symbol]: { c: quote.c, dp: quote.dp } }));
    }
  }, [quote, symbol]);
    ...
    return (
      <button key={s} onClick={() => setSymbol(s)} className={`w-full flex justify-between items-center p-3 rounded-xl transition-all border ${symbol === s ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 border-transparent hover:border-slate-200'}`}>
        <div className="text-left">
          ...
          <span className={`text-[10px] font-bold ${symbol === s ? 'text-indigo-200' : 'text-slate-400'}`}>{STOCK_METADATA[s]?.name || "Asset"}</span>
        </div>
      ...
      <nav className="bg-white/95 border-b sticky top-0 z-50 px-6 h-16 flex items-center justify-between shadow-sm backdrop-blur-md">
        <div className="flex items-center gap-10 flex-1">
          <div className="flex items-center gap-2 shrink-0">
            <div className="bg-indigo-600 p-2 rounded-xl shadow-lg"><Activity className="text-white" size={20} /></div>
        ...
        <div className="flex items-center gap-4 ml-6">
          <div className="bg-slate-100 p-1 rounded-xl flex border border-slate-200">
            {(['US', 'CN'] as MarketTab[]).map(tab => (
              <button key={tab} onClick={() => { setMarketTab(tab); setSymbol(tab === 'US' ? 'NVDA' : '600519'); }} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${marketTab === tab ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`} >{t.tabs[tab.toLowerCase() as keyof typeof t.tabs]}</button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3 pl-3 border-l border-slate-200"><div className="w-9 h-9 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-xs shadow-md">L</div><span className="text-sm font-black hidden md:block">{t.user.profile}</span></div>
        </div>
      </nav>
      <div className="max-w-[1700px] mx-auto p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <aside className="lg:col-span-3 space-y-8">
          <section className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
            <h3 className="font-black text-[10px] text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6"><Zap size={14} className="text-amber-500 fill-amber-500" /> {t.explorer}</h3>
            <div className="space-y-8">
              <div><p className="text-xs font-black text-slate-800 mb-4 flex items-center justify-between">{t.hot} <ChevronRight size={14} className="text-slate-300"/></p><div className="space-y-2">{RECOMMENDATIONS.hot.map(getSidebarItem)}</div></div>
              <div><p className="text-xs font-black text-slate-800 mb-4 flex items-center justify-between">{t.topGainers} <ChevronRight size={14} className="text-slate-300"/></p><div className="space-y-2">{RECOMMENDATIONS.gainers.map(getSidebarItem)}</div></div>
              <div><p className="text-xs font-black text-slate-800 mb-4 flex items-center justify-between">{t.chinaHot} <ChevronRight size={14} className="text-slate-300"/></p><div className="space-y-2">{RECOMMENDATIONS.ashares.map(getSidebarItem)}</div></div>
            </div>
          </section>
        </aside>
        <div className="lg:col-span-9 space-y-8">
          <article className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-10">
              <div className="space-y-4"><div className="flex items-center gap-3"><div className="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-lg uppercase tracking-widest">{marketTab} MARKET</div><h2 className="text-7xl font-black tracking-tighter text-slate-900">{symbol}</h2></div><div><p className="text-2xl font-black text-slate-400 mb-2">{STOCK_METADATA[symbol]?.name || "Asset Details"}</p><div className="flex items-baseline gap-6"><span className={`text-6xl font-black tracking-tighter transition-all duration-300 ${priceFlash === 'up' ? 'text-emerald-500 scale-105' : priceFlash === 'down' ? 'text-rose-500 scale-105' : 'text-slate-900'}`}>{formatCurrency(quote?.c)}</span><div className={`flex items-center text-lg font-black px-4 py-2 rounded-2xl ${quote && quote.d >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>{quote && (quote.d >= 0 ? <TrendingUp size={20} className="mr-2" /> : <TrendingDown size={20} className="mr-2" />)}{quote?.d?.toFixed(2)} ({quote?.dp?.toFixed(2)}%)</div></div></div></div>
              <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200">{(['1', '5', '15', '30', '60', 'D', 'W', 'M'] as Resolution[]).map(res => (<button key={res} onClick={() => setResolution(res)} className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${resolution === res ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-400 hover:text-slate-800'}`} >{t.resolutions[res]}</button>))}</div>
            </header>
            <div className="relative h-[580px] w-full rounded-3xl overflow-hidden bg-slate-50 border border-slate-100 group">{loading && <div className="absolute inset-0 bg-white/60 backdrop-blur-md z-20 flex flex-col items-center justify-center gap-4"><div className="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div><span className="text-xs font-black text-slate-400 tracking-widest uppercase">{t.syncing}</span></div>}<div className={`absolute top-4 right-4 z-10 w-3 h-3 rounded-full transition-all duration-300 ${priceFlash === 'up' ? 'bg-emerald-500 scale-150 animate-ping' : priceFlash === 'down' ? 'bg-rose-500 scale-150 animate-ping' : 'bg-slate-300'}`}></div><StockChart data={candles} /></div>
            <footer className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12 pt-10 border-t border-slate-50">{[{ label: t.stats.open, val: quote?.o, icon: <BarChart3 size={14}/> }, { label: t.stats.high, val: quote?.h, icon: <TrendingUp size={14}/> }, { label: t.stats.low, val: quote?.l, icon: <TrendingDown size={14}/> }, { label: t.stats.prevClose, val: quote?.pc, icon: <Clock size={14}/> }].map((s, idx) => (<div key={idx} className="space-y-3"><div className="flex items-center gap-2 text-slate-400">{s.icon}<p className="text-[10px] font-black uppercase tracking-[0.2em]">{s.label}</p></div><p className="text-3xl font-black text-slate-900 tracking-tight">{formatCurrency(s.val)}</p></div>))}</footer>
          </article>
        </div>
      </div>
    </main>
